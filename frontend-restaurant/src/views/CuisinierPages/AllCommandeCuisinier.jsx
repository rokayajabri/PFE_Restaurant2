...
const AllCommandeCuisinier = () => {
    ...
    const handleUpdateStatus = async (id, newStatus) => {
        ...
        try {
            await axios.patch(`http://127.0.0.1:8000/api/cuisinier/commandes/${id}/status`, { statut: newStatus }, { headers });
    ...
    const statuses = ['à traiter', 'en preparation', 'prête à servir'];
                                    ...
                                    <TableBody>
                                        {sortData(group.commandes, getComparator(order, orderBy)).map(commande => (
                                            <TableRow key={commande.id}>
                                                ...
                                                <TableCell>
                                                    {commande.statut === 'à traiter' && (
                                                        <Button
                                                            onClick={() => handleUpdateStatus(commande.id, 'en preparation')}
                                                            color="primary"
                                                            variant="contained"
                                                            size="small"
                                                    ...
                                                    {commande.statut === 'en preparation' && (
                                                        <Button
                                                            onClick={() => handleUpdateStatus(commande.id, 'prête à servir')}
                                                            color="primary"
                                                            variant="contained"
                                                            size="small"
                                                            sx={{ padding: '6px 8px', whiteSpace: 'nowrap' }}
...
export default AllCommandeCuisinier;
