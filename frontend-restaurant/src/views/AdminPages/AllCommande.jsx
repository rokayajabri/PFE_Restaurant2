    ...
    const handleDelete = async (id) => {
        if (window.confirm('Are you sure you want to delete this commande?')) {
            try {
                ...
                setLoading(true);
                await axios.delete(`http://127.0.0.1:8001/api/delete_commandes/${id}`, { headers });
    ...
    return (
        <Container>
            <Box sx={{ my: 4 }}>
                ...
                <Button component={Link} to="/addCommande" variant="contained" color="primary" sx={{ mb: 2 }}>
                            ...
                            <TableBody>
                                {sortData(commandes, getComparator(order, orderBy)).map(commande => (
                                    <TableRow key={commande.id}>
                                        ...
                                        <TableCell>
                                            <IconButton component={Link} to={`/editCommande/${commande.id}`} color="primary">
                                                <EditIcon />
                                            </IconButton>
